...
def add_time_features(daily_full, max_lag=7):
    ...
    # NUEVO: coordenadas de la celda a partir del cell_id
...
def train_random_forest(df_features, test_days=60):
    ...
    # NUEVO: usamos coordenadas + vecinos
    feature_cols = [
        'lag1', 'lag2', 'lag7', 'roll7',
        'lag1_neigh',          # crímenes de ayer en celdas vecinas
        'dow', 'month',
        'cell_x', 'cell_y'
    ]
    ...
    model = RandomForestRegressor(
        n_estimators=200,
        max_depth=15,
        n_jobs=-1,     # si te ahoga la CPU, pon 4 o 6
        random_state=42
    )
    ...
    # Tu versión de sklearn no tiene el parámetro 'squared'
    mse = mean_squared_error(y_test, pred_test)  # esto es MSE
    rmse = np.sqrt(mse)                          # aquí sacas el RMSE a mano
...
def predict_next_day(model, feature_cols, df_features):
    ...
    # NUEVO: calculamos lag1_neigh para el último día disponible
    ...
    # Ahora construimos las features para el día siguiente
    rows = []
